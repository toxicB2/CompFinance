...
// AT 20190414 WARNING: Strings are C style strings i.e. make sure the last element is being handled appropriatelly 
...
double restRestartThreadPool(double xNthread)
{
...
string restPutDupire( double              spot,
	                  vector<double>      spots,
	                  vector<double>      times,
	                  matrix<double>      vols,
	                  double              maxDt,
	                  string           xid)
{
	//  TODO Make sure we have an id and all parameters are correct
	putDupire(spot, spots, times, vols, maxDt, xid);
	return xid;
}

string restPutBlackScholes(double              spot,
	                    double              vol,
	                    double              qSpot,
	                    double              rate,
	                    double              div,
...
{
	//  TODO Make sure we have an id and all parameters are correct
	putBlackScholes(spot, vol, qSpot > 0, rate, div, xid);
	return xid;
}

string restPutEuropean(double              strike,
	                   double              exerciseDate,
	                   double              settlementDate,
	                   string              xid)
{
	//  TODO Make sure we have an id and all parameters are correct
	putEuropean(strike, exerciseDate, settlementDate, xid);

	return xid;
}

string restPutBarrier(double              strike,
	                  double              barrier,
	                  double              maturity,
	                  double              monitorFreq,
	                  double              smoothing,
...
{
	//  TODO Make sure we have an id and all parameters are correct
	putBarrier(strike, barrier, maturity, monitorFreq, smoothing, xid);

	return xid;
}

string restPutContingent( double              coupon,
	                      double              maturity,
	                      double              payFreq,
	                      double              smoothing,
	                      string          xid)
{
	//  TODO Make sure we have an id and all parameters are correct
	putContingent(coupon, maturity, payFreq, smoothing, xid);

	return xid;
}

string restPutEuropeans(vector<double>               vmaturities,
	                    vector<double>               vstrikes,
	                    string                       xid)
{
	//  TODO Make sure we have an id and all parameters are correct
	putEuropeans(vmaturities, vstrikes, xid);
	return xid;
}

string* restPayoffIds(string  xid)
{
	const  auto* prd = getProduct<double>(xid);
	//  Make sure we have a product
	//if (!prd) return TempErr12(xlerrNA);

	return const_cast<string*>(&(prd->getPayoffLabelsRef()[0]));
}

void restParameters(string  xid, string* paramLabelsOut, double* paramsTimesOut)
{
	Model<double>* mdl = const_cast<Model<double>*>(getModel<double>(xid));
	...
	paramLabelsOut = const_cast<string*>(&paramLabels[0]);
	paramsTimesOut = const_cast<Time*>(&paramsCopy[0]); //AT20190414 Convert from Time if Time is changed 

	//pair<vector<string>, vector<Time>> output(paramLabels, paramsCopy);
	//return output;
}

void restValue( string     modelid,
	            string     productid,
	            double     useSobol,
	            double     seed1,
	            double     seed2,
...
{
	Model<double>* mdl = const_cast<Model<double>*>(getModel<double>(modelid));
	//if (!mdl) return TempErr12(xlerrNA);

	const auto num = rest2num(useSobol, seed1, seed2, numPath, parallel);
	//if (!num.numPath) return TempErr12(xlerrNA);

	try
	{
		auto results = value(modelid, productid, num);
		pair<vector<string>, vector<Time>> fromlabelsAndNumbers(results.identifiers, results.values);
		paramLabelsOut = const_cast<string*>(&fromlabelsAndNumbers.first[0]);
		paramsTimesOut = const_cast<Time*>(&fromlabelsAndNumbers.second[0]); //AT WARNING Time to double
		//return fromlabelsAndNumbers;
	}
...
void restValueTime(string              modelid,
	               string              productid,
	               double              useSobol,
	               double              seed1,
	               double              seed2,
	               ...
	               double runningTime)
{
	const auto* prd = getProduct<double>(productid);
	//  Make sure we have a product
	...
	try
	{
		...
		paramsTimesOut = const_cast<Time*>(&fromlabelsAndNumbers.second[0]); //AT WARNING Time to double
		runningTime = t1 - t0;
		//pair<double, pair<vector<string>, vector<Time>>>fromTimelabelsAndNumbers(t1 - t0, fromlabelsAndNumbers);
		//return fromTimelabelsAndNumbers;
	}
	catch (const exception&)
	{
		//return TempErr12(xlerrNA);
